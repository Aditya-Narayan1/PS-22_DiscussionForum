...
export default function PostTree({ posts, onReply, threadId }) {
  ...
  if (tree.length === 0) {
    return <EmptyState icon="🌠" title="No replies yet" description="Be the first to share your thoughts." />;
  }
    ...
    <div role="list" aria-label="Discussion replies" className="space-y-1">
      {tree.map((post) => (
        <div key={post.id} role="listitem">
          <PostItem post={post} onReply={onReply} depth={0} threadId={threadId} />
        </div>
      ))}
    </div>
  );
